...
import { Box, NativeBaseProvider, Stagger, IconButton, Icon, useDisclose } from "native-base";
...
export default function OneToOneCall({ navigation }) {
	...
	const toggleVideo = async () => {
		...
		onVideo.current ? setLocalStream(myStream) : setLocalStream({ toURL: () => null });
		// myPeerConnection
		//   .getLocalStreams()[0]
		//   .getVideoTracks()[0]
		//   .onended(setRemoteStream({ toURL: () => null }));
		myPeerConnection.getLocalStreams()[0].getVideoTracks()[0].enabled = onVideo.current;
	};
